...
8. PDF 会提取可选择文本并翻译，导出时保留原 PDF 页面、图片和表格线，并在原文字坐标处覆盖写入译文；普通正文会按单页中位字号统一显示，表格内会按可用单元格宽度自动压缩译文字号，尽量避免跨格。PDF 生成时会显示加载字体、回写段落和保存文件的状态进度。扫描版 PDF 需要先 OCR，复杂 PDF 可能需要微调。
