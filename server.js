...
async function handleTranslate(req, res) {
  ...
  const timeoutId = setTimeout(() => deepseekController.abort("timeout"), DEEPSEEK_TIMEOUT_MS);
  let response;
  try {
    ...
    if (deepseekController.signal.aborted) {
      sendJson(res, 504, { error: "DeepSeek 响应超时，请稍后重试。" });
      return;
    }
    throw error;
  } finally {
