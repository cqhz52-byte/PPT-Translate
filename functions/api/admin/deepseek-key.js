...
const DEEPSEEK_KEY = "app:deepseek_api_key";

export async function onRequestGet({ request, env }) {
  if (!(await isAdmin(request, env))) return json({ error: "Unauthorized" }, 401);
  const hasSecret = Boolean(String(env.DEEPSEEK_API_KEY || "").trim());
  const hasKvKey = Boolean(env.PHONE_AUTH_KV && (await env.PHONE_AUTH_KV.get(DEEPSEEK_KEY)));
  return json({
...
export async function onRequestPost({ request, env }) {
  ...
  await env.PHONE_AUTH_KV.put(DEEPSEEK_KEY, apiKey);
...
export async function onRequestDelete({ request, env }) {
  if (!(await isAdmin(request, env))) return json({ error: "Unauthorized" }, 401);
  if (!env.PHONE_AUTH_KV) return json({ error: "PHONE_AUTH_KV is not bound." }, 501);
  await env.PHONE_AUTH_KV.delete(DEEPSEEK_KEY);
  return json({ ok: true });
}
